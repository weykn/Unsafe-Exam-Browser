...
namespace SafeExamBrowser.Runtime
{
	public class App : Application
	{
		...
		{
			var logging = File.Exists(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "log"));

			try
			{
				if (!logging) ClearData();
				StartApplication();
			}
			catch (Exception e)
			{
		...
		{
			string dataPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "SafeExamBrowser");
